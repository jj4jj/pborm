...
//file generate datetime: Sat Dec 19 13:48:54 2015
...
struct DBConf_ST {
    ...
    void		convto(DBConf & convtomsg_) const {
        ...
        convtomsg_.set_dbname(dbname.data);
    }
    void		convfrom(const DBConf & convfrommsg_) {
        ...
        strncpy(dbname.data, convfrommsg_.dbname().data(), 32-1);
    }
    int		compare(const DBConf_ST & rhs_) const {
        ...
        if (_cmp_ret != 0) { return _cmp_ret; }
        _cmp_ret = strncmp(dbname.data, rhs_.dbname.data, 32);
        if (_cmp_ret != 0) { return _cmp_ret; }
        return 0;
    }
...
struct Config_ST {
    ...
    struct {size_t count; struct { char data[32]; } list[16];}		meta_files;
    ...
    void		convto(Config & convtomsg_) const {
        ...
        assert(meta_files.count <= 16);//assertion
        for ( size_t i = 0; i < meta_files.count && i < (sizeof(meta_files.list)/sizeof(meta_files.list[0])); ++i){
            convtomsg_.add_meta_files(meta_files.list[i].data);
        }
    }
    void		convfrom(const Config & convfrommsg_) {
        ...
        assert(convfrommsg_.meta_files_size() <= 16);//assertion
        meta_files.count = 0;
        for ( size_t i = 0; i < (size_t)convfrommsg_.meta_files_size() && i < (sizeof(meta_files.list)/sizeof(meta_files.list[0])); ++i){
            strncpy(meta_files.list[i].data, convfrommsg_.meta_files(i).data(), 32-1);
            ++meta_files.count;
        }
    }
    int		compare(const Config_ST & rhs_) const {
        ...
        if (_cmp_ret != 0) { return _cmp_ret; }
        for (size_t i = 0; i < meta_files.count && i < rhs_.meta_files.count; ++i) {
            _cmp_ret = strncmp(meta_files.list[i].data, rhs_.meta_files.list[i].data, 32);
            if (_cmp_ret != 0) { return _cmp_ret; }
        }
        _cmp_ret = (meta_files.count < rhs_.meta_files.count ) ? -1 : ((meta_files.count == rhs_.meta_files.count ) ? 0 : 1);
        return 0;
    }
    ...
    int		find_idx_meta_files(const std::string & entry_) {
        auto it = std::find_if(meta_files.list, meta_files.list + meta_files.count, 
            [&entry_](decltype(meta_files.list[0]) & st) ->bool { return entry_ == st.data; }); 
        if ( it != meta_files.list + meta_files.count ) { return it - meta_files.list; }
        return -1;
    }
    int		append_meta_files(const std::string & entry_, bool shift_ = true) {
        if ( entry_.length() >= 32 ) { return -1; }
        if ( meta_files.count < 16 ) {
            strncpy(meta_files.list[meta_files.count].data, entry_.c_str(), 32-1);
            ++meta_files.count;
        }
        else {
            if ( !shift_ ) { return -2; }
            memmove(meta_files.list, meta_files.list + 1, (16-1)*sizeof(meta_files.list[0]));
            strncpy(meta_files.list[16 - 1].data, entry_.c_str(), 32-1);
        }
        return 0;
    }
    int		insert_meta_files(size_t idx_, const std::string & entry_, bool shift_force_insert_ = true) {
        if ( entry_.length() >= 32 ) { return -1; }
        if ( idx_ > meta_files.count ) { return -2; }
        if ( meta_files.count < 16 ) {
            memmove(meta_files.list + idx_ + 1, meta_files.list + idx_, (meta_files.count - idx_)*sizeof(meta_files.list[0]));
            strncpy(meta_files.list[idx_].data, entry_.c_str(), 32-1);
            ++meta_files.count;
        }
        else {
            if ( !shift_force_insert_ ) { return -3; }
            memmove(meta_files.list + idx_ + 1, meta_files.list + idx_, (16 - 1 - idx_)*sizeof(meta_files.list[0]));
            strncpy(meta_files.list[idx_].data, entry_.c_str(), 32-1);
        }
        return 0;
    }
    int		remove_meta_files(size_t idx_, bool shift_not_swap_ = true) {
        if ( idx_ >= meta_files.count ) { return -1; }
        if ( shift_not_swap_ ) {
            memmove(meta_files.list + idx_, meta_files.list + idx_ + 1, (meta_files.count - idx_ - 1)*sizeof(meta_files.list[0]));
        }
        else { meta_files.list[idx_] = meta_files.list[meta_files.count - 1]; }
        --meta_files.count;
        return 0;
    }
    int		lower_bound_meta_files(const std::string & entry_) {
        auto it = std::lower_bound(meta_files.list, meta_files.list + meta_files.count, entry_,
            [](decltype(meta_files.list[0]) & t2, const std::string & t1) ->bool { return t1 < t2.data; }); 
        if ( it != meta_files.list + meta_files.count ) { return it - meta_files.list; }
        return -1;
    }
    int		upper_bound_meta_files(const std::string & entry_) {
        auto it = std::upper_bound(meta_files.list, meta_files.list + meta_files.count, entry_,
            [](const std::string & t1,decltype(meta_files.list[0]) & t2) ->bool { return t1 < t2.data; }); 
        if ( it != meta_files.list + meta_files.count ) { return it - meta_files.list; }
        return -1;
    }
    int		binsert_meta_files(const std::string & entry_, bool shift_force_insert_ = false) {
        int idx_ = upper_bound_meta_files(entry_);
        if ( idx_ < 0 ) { return append_meta_files(entry_, shift_force_insert_); }
        return insert_meta_files(idx_, entry_, shift_force_insert_);
    }
};
