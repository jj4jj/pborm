...
//file generate datetime: Thu Dec 17 02:29:47 2015
...
struct Config_ST {
    ...
    void		convto(Config & convtomsg_) const {
        ...
        convtomsg_.set_meta_file(meta_file.data);
    }
    void		convfrom(const Config & convfrommsg_) {
        ...
        strncpy(meta_file.data, convfrommsg_.meta_file().data(), 32-1);
    }
    int		compare(const Config_ST & rhs_) const {
        ...
        if (_cmp_ret != 0) { return _cmp_ret; }
        _cmp_ret = strncmp(meta_file.data, rhs_.meta_file.data, 32);
        if (_cmp_ret != 0) { return _cmp_ret; }
        return 0;
    }
